...
# ===
# GSE117931
assay = fread('benchmark/extdata/multi-omic/GSE117931/ADEx_data/expression.tsv',
              header = T, data.table = F)
rownames(assay) = assay[,1]; assay = assay[, -1]

meta = read.delim2(
  'benchmark/extdata/multi-omic/GSE117931/ADEx_data/metadata.tsv',
  header = T)
...
# GSE82221
assay = fread('benchmark/extdata/multi-omic/GSE82221/ADEx_data/expression.tsv',
              header = T, data.table = F)
rownames(assay) = assay[,1]; assay = assay[, -1]
...
# ===
# GSE117931
assay = fread('benchmark/extdata/multi-omic/GSE117931/ADEx_data/methylation.tsv',
              header = T, data.table = F)
rownames(assay) = assay[,1]; assay = assay[, -1]
assay = na.omit(assay)
...
# GSE82221
assay = fread('benchmark/extdata/multi-omic/GSE82221/ADEx_data/methylation.tsv',
              header = T, data.table = F)
rownames(assay) = assay[,1]; assay = assay[, -1]
assay = na.omit(assay)
assay = as.data.frame(t(apply(assay, 1, function(x) log2(x / (1-x)))))
